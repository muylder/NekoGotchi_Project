#include <M5Cardputer.h>#include <M5Unified.h>/*

#include <M5Unified.h>

#include <M5Cardputer.h> * M5Gotchi PRO - WiFi Pentest Edition

namespace {

constexpr uint16_t kBgColor = TFT_BLACK; * Build funcional com WiFi scanning e ataques básicos

constexpr uint16_t kFgColor = TFT_GREEN;

constexpr uint32_t kHeartbeatIntervalMs = 1000;namespace { */
...
void setup() {
    ...
    Serial.begin(115200);constexpr uint16_t kFgColor = TFT_GREEN;#include <M5Unified.h>
    ...
    Serial.println(F("========================"));#include <WiFi.h>

    Serial.println(F("Stage 1: Core Boot Test"));

    Serial.println(F("========================"));void setup() {#include <esp_wifi.h>



    auto cfg = M5.config();    Serial.begin(115200);#include <WebServer.h>

    M5.begin(cfg);

    Serial.println(F("[1/4] M5.begin ✓"));    delay(200);#include <DNSServer.h>
    ...
    Serial.println(F("========================"));#include <FS.h>

    M5.Display.setRotation(1);

    M5.Display.setTextSize(1);    Serial.println(F("Stage 1: Core Boot Test"));#include "esp_wifi_types.h"

    M5.Display.setTextColor(kFgColor, kBgColor);

    M5.Display.fillScreen(kBgColor);    Serial.println(F("========================"));

    Serial.println(F("[3/4] Display init ✓"));

// Hardware modules

    M5.Display.setCursor(20, 36);

    M5.Display.setTextSize(2);    Serial.println(F("[1/4] M5.begin"));#include <IRremoteESP8266.h>

    M5.Display.println(F("M5Gotchi PRO"));

    auto cfg = M5.config();#include <IRsend.h>

    M5.Display.setCursor(20, 64);

    M5.Display.setTextSize(1);    M5.begin(cfg);#include <IRrecv.h>

    M5.Display.println(F("Stage 1: Core Boot"));

    Serial.println(F("        ✓ M5.begin OK"));#include <IRutils.h>

    M5.Display.setCursor(20, 80);

    M5.Display.println(F("Waiting..."));#include <RCSwitch.h>

    Serial.println(F("[4/4] Screen draw ✓"));

    Serial.println(F("[2/4] M5Cardputer.begin"));#include <SPI.h>

    Serial.println(F("Setup complete. Loop heartbeat running."));

}    M5Cardputer.begin();#include <LoRa.h>



void loop() {    Serial.println(F("        ✓ Cardputer OK"));#include <TinyGPSPlus.h>
...
#include <HardwareSerial.h>
    ...
    const uint32_t now = millis();    Serial.println(F("[3/4] Display setup"));#include "m5gotchi_rfid_nfc.h"

    if (now - lastLog >= kHeartbeatIntervalMs) {

        Serial.println(F("Heartbeat: M5 core alive"));    M5.Display.setRotation(1);#include "bluetooth_attacks.h"

        lastLog = now;

    }    M5.Display.setTextSize(1);#include "m5gotchi_neko_virtual_pet.h"



    delay(10);    M5.Display.setTextColor(kFgColor, kBgColor);#include "m5gotchi_tutorial_system.h"

}

    M5.Display.fillScreen(kBgColor);#include "m5gotchi_achievement_manager.h"

    Serial.println(F("        ✓ Display OK"));

// ==================== CONFIGURAÇÕES ====================

    Serial.println(F("[4/4] Drawing message"));#define SCREEN_WIDTH 240

    M5.Display.setCursor(20, 40);#define SCREEN_HEIGHT 135

    M5.Display.setTextSize(2);#define MAX_NETWORKS 20

    M5.Display.println("M5Gotchi PRO");
    ...
    M5.Display.println("Waiting...");#define SD_CS   34

    Serial.println(F("        ✓ Screen updated"));

// Evil Portal

    Serial.println(F("Setup complete. Loop heartbeat running."));#define DNS_PORT 53
...
void loop() {
    ...
    M5.update();// Handshake Capture

#define EAPOL_FRAME 0x888E

    static uint32_t lastLog = 0;#define SNAP_LEN 2324

    const uint32_t now = millis();

    if (now - lastLog >= 1000U) {// File paths

        Serial.println(F("Heartbeat: M5 core alive"));#define HANDSHAKE_DIR "/handshakes"

        lastLog = now;#define PORTAL_DIR "/portals"
